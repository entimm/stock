...
class Strategy:
    ...
    def run(self, symbol):
        ...
        for index, row in df.iterrows():
            ...
            # 执行卖出计划
            if self.hold_stock and self.is_plan_sell:
                self.sell(row['date'], row['open'])

            if (not self.hold_stock) and self.is_plan_buy:
                # 当天最低价高于基准价就执行买入计划
            ...
            if self.hold_stock:
                ...
                if self.buy_ts.date() < row['date'].date() and row['low'] < self.sell_base_price:
                    self.sell(row['date'], self.sell_base_price)
                    self.sell_flag = 0
        ...
        # 结果输出
        result_json_file = os.path.join(RESOURCES_PATH, f'back_test_{symbol}.json')
    ...
    def make_plan(self, row, fractal):
        ...
        if fractal and fractal.fractal_type == FractalType.BOTTOM:
            ...
            # 死叉卖，维护基准价格
            if not self.hold_stock and (not row['cross']) and self.can_buy(row):
        ...
        if self.hold_stock:
            # 金叉后遇到顶分型
            if fractal and fractal.fractal_type == FractalType.TOP and row['cross']:
    ...
    @staticmethod
    def can_buy(row):
        return row['ma432_angle'] > 0 and row['ma15'] >= row['ma432'] and row['ma60'] >= row['ma432'] and row['low'] > row['ma432']
